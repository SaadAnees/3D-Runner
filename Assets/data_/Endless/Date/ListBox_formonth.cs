...
public class ListBox_formonth : MonoBehaviour
{
    public int listBoxID;
    // Must be unique, and count from 0
    public Text content;
    // The content of the list box

    public ListBox_formonth lastListBox;
    public ListBox_formonth nextListBox;
    ...
    void initialContent()
    {
        if (listBoxID == ListPositionCtrl.Instance.listBoxes.Length / 2)
            ...
            _contentID = ListBank.Instance.getListLength_years() - (ListPositionCtrl.Instance.listBoxes.Length / 2 - listBoxID);
        else
            _contentID = listBoxID - ListPositionCtrl.Instance.listBoxes.Length / 2;

        while (_contentID < 0)
            _contentID += ListBank.Instance.getListLength_years();
        _contentID = _contentID % ListBank.Instance.getListLength_years();

        updateContent(ListBank.Instance.getListContent_years(_contentID));
        //updateContent (ListBank.Instance.getListLength_years (_contentID).ToString ());
    ...
    */
    ...
    void updateToLastContent()
    {
        ...
        _contentID = (_contentID < 0) ? ListBank.Instance.getListLength_years() - 1 : _contentID;
    ...
    void updateToNextContent()
    {
        ...
        _contentID = (_contentID == ListBank.Instance.getListLength_years()) ? 0 : _contentID;

        updateContent(ListBank.Instance.getListContent_years(_contentID));
    }
    public void GetData()
    {
        print(content.text.ToString());
    }
}
